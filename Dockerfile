...
RUN rosdep update && apt-get update && apt-get update --fix-missing && apt-get upgrade -y
COPY ./root_dir/ros2_ws /root/ros2_ws
RUN cd /root/ros2_ws/ && . /opt/ros/humble/setup.sh && rosdep install --from-paths src --ignore-src -r -y
